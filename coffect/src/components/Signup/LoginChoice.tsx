...
const LoginChoice: React.FC<LoginChoiceProps> = ({ onSignUp, onLogin }) => {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-start bg-white px-[1%] pt-[35%] text-center">
      {/* 추후 로고로 수정 예정 */}
      <h1 className="mb-[2%] text-5xl font-bold text-gray-900">coffect</h1>
      {/* 설명 문구 */}
      <p className="mb-[25%] text-lg font-medium text-[#4A4A4A]">
        공강 시간을 더 생산적이게
      </p>
      {/* 아이디/비밀번호 입력창 */}
      <input
        ...
        className="mb-[3%] w-full max-w-[90%] rounded bg-gray-100 px-[4%] py-[3%] text-base text-gray-700"
      />
      <input
        type="password"
        placeholder="비밀번호"
      ...
      <button
        ...
        className="w-full max-w-[90%] rounded-lg bg-black py-[4%] text-white"
      >
        로그인
      </button>
      ...
      <button
        onClick={onSignUp}
        className="w-full max-w-[90%] rounded-lg border border-[#ACACAC] py-[4%] text-[#49495A]"
      >
        학교 이메일로 가입하기
      </button>
    </div>
