...
const PostTags: React.FC<PostTagsProps> = ({ type, topic }) => {
  ...
  const displayType =
    postTypeOptions.find((option) => option.value === type)?.label || type;
  return (
    <div className="my-3 flex gap-1.5">
      {displayType && <span className={`${TagStyle}`}>{displayType}</span>}

      {topic &&
        topic.map((subject, index) => {
          const displaySubject =
            postSubjectOptions.find((option) => option.value === subject)
              ?.label || subject;
          return (
