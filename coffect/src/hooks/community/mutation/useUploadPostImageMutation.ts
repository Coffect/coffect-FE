/**
 * @file useUploadPostImageMutation.ts
 * @author 흥부/강신욱
 * @description
 * 게시글 "이미지" 업로드를 위한 React Query Mutation 훅입니다.
 * 이 훅은 게시글 작성 시 이미지 파일을 서버에 업로드하는 기능을 제공합니다.
 * @version 1.0.0
 * - 1.0.0 : 초기 작성 ( 게시글 이미지 업로드 Mutation 훅 정의 )
 * @date 2023-08-05
 */
 ...
 * @property {PostImageUploadResponse} data - 이미지 업로드에 성공했을 때 반환되는 데이터 객체
 * @property {string} data.resultType - 결과 타입 (예: "SUCCESS", "FAIL")
 * @property {null | { errorCode: string, reason: string, data?: null }} data.error - 에러 정보 (실패 시)
 * @property {null | { imageUrl: string }} data.success - 성공 정보 (성공 시, 업로드된 이미지 URL 포함)
 * @property {function} mutateAsync - 이미지 업로드를 수행하는 함수
 ...
 * @property {PostImageUploadResponse | null} data - 업로드 성공 시 반환되는 데이터 (isSuccess가 true일 때)
 */
